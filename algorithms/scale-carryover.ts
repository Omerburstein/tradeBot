...
/**
 * The factor the two headline charts plot. Gamma is the anchor of the whole
 * signal (wGex is the largest weight), so it gets the charts; the other three
 * factors stay in the table view rather than diluting the question into four
 * facets that are read four times.
 */
const CHART_FACTOR: Factor = 'gexRaw';
...
function main(): Promise<void> {
  return collectRatios().then(({ ratios, dates, skipped }) => {
    ...
    const meta = {
      ...
      chartFactor: CHART_FACTOR,
      chartFactorLabel: FACTOR_LABELS[CHART_FACTOR],
    };
...
<div class="viz-root">
<div class="wrap">
  <h1>Can the previous day seed gamma's opening scale?</h1>
  <p class="sub">
    Gamma is normalized by <code>meanAbs</code> of its recent history, which needs 3 fresh Greek ticks —
    30 minutes of wall clock. Seeding that scale from the previous day would remove the warm-up, but only if the
    previous day's gamma magnitude actually predicts the next open. Each bar counts trading days whose ratio
    (previous-day magnitude &divide; current-day opening magnitude) fell in that bin.
    <b>1&times; is a perfect seed</b>; the gap between the <b>1&times;</b> line and the <b>typical</b> line is the
    seed's bias. Bins are one doubling wide.
  </p>
  ...
  <details>
    <summary>Table view — all four factors, summary statistics and full bin counts</summary>
    <div id="tables"></div>
  </details>
</div>
</div>
...
const W = 540, H = 300, M = { top: 46, right: 14, bottom: 42, left: 38 };
const PW = W - M.left - M.right, PH = H - M.top - M.bottom;

const meta = DATA.meta;
document.getElementById('meta').textContent =
  meta.days + ' trading days (' + meta.firstDate + ' \\u2192 ' + meta.lastDate + ') \\u00b7 ' +
  'charts show ' + meta.chartFactorLabel + ' only \\u00b7 comparing the first/last ' + meta.edgeTicks +
  ' fresh Greek ticks of each day';

const fmtRatio = (k) => k < 0 ? '1/' + Math.pow(2, -k) + '\\u00d7' : Math.pow(2, k) + '\\u00d7';
const nBins = meta.binMax - meta.binMin;

/** One single-series histogram. si picks the palette slot (0 = open, 1 = close). */
function chartSvg(ser, si) {
  const maxCount = Math.max(1, ...ser.bins);
  ...
  const barW = PW / nBins - gap;
  let s = '';

  // "Within 2x" band — the usable-seed zone the stat line quotes.
  s += '<rect x="' + x(-1 - meta.binMin) + '" y="' + M.top + '" width="' + (2 * PW / nBins) +
       '" height="' + PH + '" fill="var(--grid)" opacity="0.5"/>';
  ...
  ser.bins.forEach((c, b) => {
    ...
    if (c > 0) {
      ...
      s += '<rect class="bar" x="' + bx.toFixed(2) + '" y="' + y(c).toFixed(2) + '" width="' + barW.toFixed(2) +
           '" height="' + h.toFixed(2) + '" rx="4" fill="' + COLORS[si] + '"/>';
    }
    // Hit target spans the full column height (>= 24px) regardless of value.
    s += '<rect class="hit" x="' + (bx - gap / 2).toFixed(2) + '" y="' + M.top + '" width="' + (barW + gap).toFixed(2) +
         '" height="' + PH + '" data-s="' + si + '" data-b="' + b + '"/>';
  });

  // Two reference lines: 1x (perfect seed) and the actual geomean. The GAP
  // between them is the seed's bias — the point of the chart. Labels are
  // stacked at different heights above the plot so they stay readable even
  // when the two lines coincide (the open seed sits at 0.99x, i.e. on 1x).
  const clampX = (px, w) => Math.max(M.left, Math.min(px, M.left + PW - w));
  const zx = x(-meta.binMin);
  const gx = x(Math.log2(ser.stats.geomean) - meta.binMin);
  // Each rule gets a surface ring so it stays legible where it crosses bars —
  // the geomean line is the series' own hue and would otherwise vanish into them.
  const rule = (px, color) =>
    '<line x1="' + px + '" x2="' + px + '" y1="' + (M.top - 8) + '" y2="' + (M.top + PH) +
    '" stroke="var(--surface-1)" stroke-width="4.5"/>' +
    '<line x1="' + px + '" x2="' + px + '" y1="' + (M.top - 8) + '" y2="' + (M.top + PH) +
    '" stroke="' + color + '" stroke-width="1.5"/>';
  s += rule(zx.toFixed(2), 'var(--text-secondary)');
  s += '<text class="tick" x="' + clampX(zx + 5, 74) + '" y="' + (M.top - 26) +
       '" fill="var(--text-secondary)">1\\u00d7 perfect seed</text>';
  s += rule(gx.toFixed(2), COLORS[si]);
  s += '<text class="tick" x="' + clampX(gx + 5, 84).toFixed(2) + '" y="' + (M.top - 12) + '" fill="' + COLORS[si] +
       '">' + ser.stats.geomean.toFixed(2) + '\\u00d7 typical</text>';
  ...
  s += '<text class="tick" x="' + (M.left + PW / 2) + '" y="' + (M.top + PH + 34) +
       '" text-anchor="middle">previous-day gamma \\u00f7 current-day opening gamma</text>';
...
// Two charts, gamma only: one seeded from the previous day's open, one from its
// close. Single series each, so no legend box — the card title names it.
const CHARTS = [
  { si: 0, title: "Seeded from the previous day's OPEN", sub: 'first 3 fresh ticks of the previous session' },
  { si: 1, title: "Seeded from the previous day's CLOSE", sub: 'last 3 fresh ticks of the previous session' },
];
const gamma = DATA.facets.find(f => f.factor === meta.chartFactor);
document.getElementById('grid').innerHTML = CHARTS.map(c => {
  const ser = gamma.series[c.si];
  const note = '<b style="color:' + COLORS[c.si] + '">' + ser.stats.geomean.toFixed(2) + '\\u00d7</b> typical &nbsp;\\u00b7&nbsp; ' +
    '<b>' + Math.round(ser.stats.within2x * 100) + '%</b> within 2\\u00d7 &nbsp;\\u00b7&nbsp; ' +
    'p10\\u2013p90 ' + ser.stats.p10.toFixed(2) + '\\u00d7 to ' + ser.stats.p90.toFixed(2) + '\\u00d7 &nbsp;\\u00b7&nbsp; n=' + ser.stats.n;
  return '<div class="card"><h2>' + c.title + '</h2><p class="note">' + c.sub + '</p><p class="note">' + note + '</p>' +
    chartSvg(ser, c.si) + '</div>';
}).join('');
